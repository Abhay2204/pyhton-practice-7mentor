"""
app.py
Main Backend Application and Web Server.
Uses Python's built-in http.server to serve the HTML/CSS/JS frontend and handle API requests.
...
class PortalRequestHandler(http.server.SimpleHTTPRequestHandler):
    """Custom request handler that serves static frontend files and processes JSON APIs."""
    ...
    def do_GET(self):
        """Handle API GET calls or serve static frontend files."""
        ...
        # Default: Serve static HTML/CSS/JS frontend
        super().do_GET()

    def do_POST(self):
        """Handle API POST calls for login, registration, and management."""
        parsed = urllib.parse.urlparse(self.path)
        path = parsed.path
        ...
        # 7. Reset DB to default demo data
...
def run_server():
    """Starts the local web server and prints instructions."""
